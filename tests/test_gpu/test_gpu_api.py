...
from alchemy.gpu.monitor import GPUInfo, HardwareSnapshot, RAMInfo
...
async def test_get_status(client):
    resp = await client.get("/v1/gpu/status")
...
async def test_get_gpus(client):
    resp = await client.get("/v1/gpu/gpus")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 2
...
async def test_get_ram(client):
    resp = await client.get("/v1/gpu/ram")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_get_models(client):
    resp = await client.get("/v1/gpu/models")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
async def test_get_model_by_name(client):
    resp = await client.get("/v1/gpu/models/qwen3:14b")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_get_model_not_found(client):
    resp = await client.get("/v1/gpu/models/nonexistent")
...
async def test_promote_model(client):
    ...
    resp = await client_fixture.post("/v1/gpu/models/qwen3:14b/promote")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_demote_model(client):
    ...
    resp = await client.post("/v1/gpu/models/qwen3:14b/demote")
    assert resp.status_code == 200
...
async def test_load_model(client):
    ...
    resp = await client.post("/v1/gpu/models/qwen3:14b/load")
    assert resp.status_code == 200
    data = resp.json()
    assert data["success"] is True
...
async def test_rebalance(client):
    ...
    resp = await client.post("/v1/gpu/rebalance")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_app_activate(client):
    ...
    resp = await client.post("/v1/gpu/app/spotify/activate",
...
async def test_app_deactivate(client):
    ...
    resp = await client.post("/v1/gpu/app/spotify/deactivate")
    assert resp.status_code == 200
    data = resp.json()
    assert data["success"] is True
