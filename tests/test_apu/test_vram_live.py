...
class TestVRAMLeak:
    ...
    async def test_load_unload_small_model_vram_returns(self):
        ...
        if r.status_code != 200:
            print(f"  Load response: {r.text[:300]}")
    ...
    async def test_small_model_evicted_when_large_needs_space(self):
        """Load small model, then large -> small should be evicted."""
        r = await load_model(SMALL_MODEL)
        print(f"\n  Loaded {SMALL_MODEL}: status={r.status_code}")

        t0 = time.perf_counter()
        r = await load_model(LARGE_MODEL)
        elapsed = time.perf_counter() - t0
        print(f"  Load {LARGE_MODEL}: {elapsed:.1f}s, status={r.status_code}")
...
class TestFrozenRoutine:
    ...
    async def test_apu_health_and_events(self):
        """APU health and event tracking works."""
        t0 = time.perf_counter()
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.get(f"{ALCHEMY_URL}/v1/apu/status")
        elapsed = time.perf_counter() - t0
        ...
        async with httpx.AsyncClient(timeout=10) as c:
            r = await c.get(f"{ALCHEMY_URL}/v1/apu/events")
        if r.status_code == 200:
            print(f"  Events endpoint: {len(r.json())} events")
