...
Centralized test results for both **Alchemy** and **NEOSY** repos.

## How It Works

1. **Run tests** — `bash testing/run_tests.sh` (or run per-repo manually)
2. **Results land here** — `testing/results/alchemy_latest.md` and `testing/results/neosy_latest.md`
...
# NEOSY (from BaratzaMemory root)
cd ~/BaratzaMemory
PYTHONPATH=src pytest tests/ --tb=short -q 2>&1 | tee ~/Documents/Alchemy_explore/testing/results/neosy_latest.txt
```
